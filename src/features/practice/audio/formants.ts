    ...
    const candidates = roots
      ...
      .map((root) => {
        ...
        const bandwidth =
          (-0.5 *
            sampleRate *
            Math.log(root.re * root.re + root.im * root.im)) /
          Math.PI;
        return { frequency, bandwidth };
      })
      .filter(
        ({ frequency, bandwidth }) =>
          frequency > 180 && frequency < 3600 && bandwidth < 900,
      )
      .sort((a, b) => a.frequency - b.frequency);

    const f1 = candidates[0]?.frequency;
    const f2 = candidates.find(
      (candidate) => candidate.frequency > (f1 ?? 0) + 250,
    )?.frequency;
    if (f1 && f2) {
      frames.push({
...
function preEmphasize(frame: Float32Array): Float32Array {
  ...
  for (let i = 1; i < frame.length; i += 1)
    output[i] = frame[i] - PRE_EMPHASIS * frame[i - 1];
  return output;
}
...
function lpc(frame: Float32Array, order: number): number[] {
  const autocorrelation = Array.from({ length: order + 1 }, (_, lag) => {
    let sum = 0;
    for (let i = lag; i < frame.length; i += 1)
  ...
  for (let i = 1; i <= order; i += 1) {
    ...
    for (let j = 1; j < i; j += 1)
      accumulator += coefficients[j] * autocorrelation[i - j];
    const reflection = -accumulator / error[i - 1];
    const next = coefficients.slice();
    next[i] = reflection;
    for (let j = 1; j < i; j += 1)
      next[j] = coefficients[j] + reflection * coefficients[i - j];
  ...
  for (let iteration = 0; iteration < 60; iteration += 1) {
    roots = roots.map((root, index) => {
      ...
      const denominator = roots
        ...
        .reduce<Complex>(
          (product, other) => multiply(product, subtract(root, other)),
          { re: 1, im: 0 },
        );
...
function evaluatePolynomial(coefficients: number[], z: Complex): Complex {
  return coefficients.reduce<Complex>(
    (value, coefficient) => add(multiply(value, z), { re: coefficient, im: 0 }),
    {
      re: 0,
      im: 0,
    },
